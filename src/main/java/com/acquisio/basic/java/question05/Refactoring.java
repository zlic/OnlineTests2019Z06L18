...
public class Refactoring {
    ...
    private void increaseQuality(Item item, Integer number) {
        if (number > 0 && item.quality + number < 51) {
            item.quality = item.quality + number;
        }
    }

    private void decreaseQuality(Item item, Integer number) {
        if (number > 0 && item.quality - number > -1) {
            item.quality = item.quality - number;
        }
    }
}